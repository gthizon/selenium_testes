...
public class TesteAgendador extends TesteGenerico {
	...
	@Test(testName = "id: 15 - Verificar se não permitirá abrir janela periodicidade caso o registro não esteja informado", dependsOnMethods = {
			"cadastrarNovoAgendamento" })
	public void verificarSeNaoPermiteAbrirJanelaComRegistroNaoInformado() {
		acao.limparElemento(nomeAgendamento);
		acao.clicarNoElemento(rotina, botaoAbrirConfiguracoesPeriodo);
		msg.aguardarMensagem("Informe o nome");
		acao.clicarNoElemento(rotina, msg.okNotificacao);
		espera.aguardarElementoDesaparecer(msg.okNotificacao);
		rotina.aguardarProcessarRotina();
		rotina.cancelarAlteracao();
		acao.clicarNoElemento(rotina, rotina.botaoLimparConsulta);
		rotina.aguardarProcessarRotina();
		acao.escreverNoElemento(idAgendamento, codigoAgendamento);
		acao.clicarNoElemento(rotina, rotina.botaoConsultar);
		rotina.aguardarProcessarRotina();
	}

	@Test(testName = "id: 3 - Vincular relatorio ao agendamento", dependsOnMethods = {
			"verificarSeNaoPermiteAbrirJanelaComRegistroNaoInformado" })
	...
	public void executarAgendamento() {
		acao.clicarNoElemento(rotina, botaoExecutarAgendamento);
		msg.aguardarMensagem("executado.");
